...
class Dialplan:
    ...
    def generate_public_xml(self, mobile):
        data = [('sys_bridge', False,
            [('destination_number', '^(.*)$', False, [
                ('bridge', 'sofia/external/{0}@{1}'.format(mobile, fs_settings.DEFAULT_GATEWAY_REAML), False),
            ])])]
        xml = Context(self.context)
        self._generate_xml(self.xml, data)
        return xml
    ...
    def to_xml(self, data):
        xml = XMLCurlFactory(data=data, section=self.section)
        return xml.convert()
